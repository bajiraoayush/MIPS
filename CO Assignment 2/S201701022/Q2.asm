...
.text
	main:
		...
		while_main:				#While loop.
			beqz $v0, exit			#Loop condition.
			
			div $v0, $v0, $t0
			addi $s1, $s1, 1
			
			b while_main			#Bracnhing while.
		exit:
			#li $v0, 10
			#syscall
		...
		beq $s0, $s2, True			#Final condition t check whether sum is equal to number.
		bne $s0, $s2, False
		
		True:
			li $v0, 4			#Printing message if number is Armstrong number.
			la $a0, isArm
			syscall
			li $v0, 10
			syscall
		...
		False:
			li $v0, 4			#Printing message if number is not Armstrong number.
			la $a0, isNotArm
			syscall
			li $v0, 10
			syscall
	...
	power:						#Power function.
		while_power:					
			beqz $a2, exit_power		#Loop condition.
			mult $a1, $t1			#
